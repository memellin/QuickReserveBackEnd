...
import com.example.demo.domain.people.PeopleNotFoundException;
...
//badge eh o cracha
    public PeopleBadgeResponseDTO getPeopleBadge(String peopleId, UriComponentsBuilder uriComponentsBuilder){
        People people = this.peopleRepository.findById(peopleId).orElseThrow(() -> new PeopleNotFoundException("pessoa nao encontrada com o id" + peopleId));
        var uri = uriComponentsBuilder.path("/peoples/{peopleId}/check-in").buildAndExpand(peopleId).toUri().toString();
        PeopleBadgeDTO peopleBadgeDTO = new PeopleBadgeDTO(people.getName(), people.getEmail(), uri, people.getSchool().getId());
